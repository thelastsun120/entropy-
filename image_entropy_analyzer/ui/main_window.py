...
from core.entropy_image_generator import generate_image_with_target_entropy
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self) -> None:
        ...
        self.generate_target_entropy_btn = QPushButton("生成指定熵图像")
        ...
        self.llm_status_label = QLabel("LLM：未启用")
        ...
        action_layout.addWidget(self.generate_target_entropy_btn)
        action_layout.addWidget(self.clear_btn)
        action_layout.addWidget(self.save_btn)
        action_layout.addWidget(self.llm_settings_btn)
        ...
        display_layout.addWidget(self.llm_status_label)
    ...
    def connect_signals(self) -> None:
        ...
        self.generate_target_entropy_btn.clicked.connect(self.generate_target_entropy_image)
    ...
    def open_llm_settings(self) -> None:
        dialog = LLMSettingsDialog(self.llm_enabled, self.llm_api_key, self)
        if dialog.exec_():
            enabled, api_key = dialog.get_values()
            self.llm_enabled = enabled
            self.llm_api_key = api_key
            self.llm_status_label.setText("LLM：已启用" if enabled else "LLM：未启用")
    ...
    def generate_target_entropy_image(self) -> None:
        if self.gray_image is None:
            QMessageBox.warning(self, "提示", "请先上传图像")
            return

        target_str, ok = QInputDialog.getText(self, "生成指定熵图像", "请输入目标熵值（0~8）：")
        if not ok:
            return
        try:
            target_entropy = float(target_str)
        except ValueError:
            QMessageBox.warning(self, "提示", "请输入有效数字")
            return
        if not (0.0 <= target_entropy <= 8.0):
            QMessageBox.warning(self, "提示", "目标熵值范围应在 0~8")
            return

        generated, achieved = generate_image_with_target_entropy(self.gray_image, target_entropy)
        self.noisy_image = generated
        self.noisy_entropy = achieved
        self.processed_panel.set_image(generated)
        self.comparison_canvas.plot_entropy_comparison(
            calculate_global_entropy(self.gray_image),
            achieved,
        )
        self.result_text.append(f"目标熵值：{target_entropy:.4f} bit")
        self.result_text.append(f"生成图像熵值：{achieved:.4f} bit")
        self.result_text.append(f"偏差：{(achieved - target_entropy):+.4f} bit")
        self.result_text.append("说明：已基于上传图像生成接近目标熵的新图像。")
        self.result_text.append("")
...
class LLMSettingsDialog(QDialog):
    def __init__(self, enabled: bool, api_key: str, parent=None) -> None:
        super().__init__(parent)
        self.setWindowTitle("LLM 设置")
        self.setMinimumWidth(420)

        layout = QVBoxLayout(self)
        self.enable_cb = QCheckBox("启用 LLM 复杂度评级")
    ...
    def get_values(self) -> tuple[bool, str]:
        return self.enable_cb.isChecked(), self.api_key_input.text().strip()
